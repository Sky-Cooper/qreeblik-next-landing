...
const CTASection = () => {
  ...
  return (
    <section className="relative pt-32 pb-40 bg-gradient-to-br from-indigo-900 via-blue-800 to-cyan-700 text-white overflow-hidden">
      <motion.div className="absolute top-0 left-0 w-[28rem] h-[28rem] bg-cyan-500/30 rounded-full blur-3xl -translate-x-40 -translate-y-40" animate={{ y: [0, -20, 0], scale: [1, 1.05, 1], transition: { duration: 10, repeat: Infinity, ease: 'easeInOut' } }} />
      <motion.div className="absolute bottom-0 right-0 w-[28rem] h-[28rem] bg-indigo-500/30 rounded-full blur-3xl translate-x-32 translate-y-32" animate={{ y: [0, 20, 0], scale: [1, 1.05, 1], transition: { duration: 12, repeat: Infinity, ease: 'easeInOut' } }} />
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
            <motion.div className="text-center lg:text-left" variants={sectionVariants} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.4 }}>
                <motion.h2 variants={sectionVariants} className="text-4xl md:text-5xl font-extrabold tracking-tighter" style={{ textShadow: '0 2px 10px rgba(0,0,0,0.2)' }}>
                    Rejoignez la nouvelle ère de la santé
                </motion.h2>
                <motion.p variants={sectionVariants} className="mt-4 text-lg text-indigo-100/90 max-w-lg mx-auto lg:mx-0">
                    Que vous soyez médecin ou patient, Qreeb Lik est la solution tout-en-un pour une expérience de soin simplifiée et efficace.
                </motion.p>
                <motion.div variants={sectionVariants} className="mt-10 flex flex-col items-center lg:items-start gap-8">
                    <div>
                         <h3 className="text-xl font-semibold text-cyan-300">Pour les Médecins</h3>
                         <p className="text-indigo-200 mb-4">Optimisez votre cabinet et gagnez du temps.</p>
                         <a href="https://dashboard.qreeblik.com/login" className="group inline-block px-8 py-4 bg-white text-blue-700 font-semibold rounded-full shadow-lg hover:shadow-2xl transition-all hover:-translate-y-1 duration-300">
                             Essayer Qreeb Lik
                         </a>
                    </div>
                    <div>
                        <h3 className="text-xl font-semibold text-cyan-300">Pour les Patients</h3>
                        <p className="text-indigo-200 mb-4">Votre santé, à portée de main.</p>
                    ...
                    <div className="w-full h-full bg-black rounded-md overflow-hidden relative">
                         <AnimatePresence>
                             <motion.div key={currentPcScreen} variants={pcSlideVariants} initial="enter" animate="center" exit="exit" className="absolute inset-0">
                                <Image src={pcScreens[currentPcScreen]} alt="Dashboard médecin" fill style={{ objectFit: 'cover' }} priority />
                             </motion.div>
                         </AnimatePresence>
                    </div>
                </div>
                    ...
                    <div className="w-full h-full bg-black rounded-[24px] overflow-hidden relative">
                         <AnimatePresence>
                             <motion.div key={currentMobileScreen} variants={mobileSlideVariants} initial="enter" animate="center" exit="exit" className="absolute inset-0">
                                <Image src={mobileScreens[currentMobileScreen]} alt="Application patient" fill style={{ objectFit: 'cover' }} priority />
                             </motion.div>
                         </AnimatePresence>
                    </div>
                </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-40 overflow-hidden">
        <svg viewBox="0 0 1440 160" className="w-full h-full" preserveAspectRatio="none">
          <motion.path fill="#1e293b" fillOpacity="0.8" d="M0,140 C350,220 700,60 1050,140 C1250,190 1350,100 1440,140 L1440,160 L0,160Z" animate={{ d: ["M0,140 C350,220 700,60 1050,140 C1250,190 1350,100 1440,140 L1440,160 L0,160Z", "M0,150 C350,210 700,70 1050,150 C1250,200 1350,90 1440,150 L1440,160 L0,160Z", "M0,140 C350,220 700,60 1050,140 C1250,190 1350,100 1440,140 L1440,160 L0,160Z"], transition: { duration: 8, repeat: Infinity, ease: "easeInOut" } }} />
          <motion.path fill="#0f172a" fillOpacity="1" d="M0,120 C300,200 600,40 900,120 C1150,180 1300,80 1440,120 L1440,160 L0,160Z" animate={{ d: ["M0,120 C300,200 600,40 900,120 C1150,180 1300,80 1440,120 L1440,160 L0,160Z", "M0,130 C300,190 600,50 900,130 C1150,190 1300,90 1440,130 L1440,160 L0,160Z", "M0,120 C300,200 600,40 900,120 C1150,180 1300,80 1440,120 L1440,160 L0,160Z"], transition: { duration: 10, repeat: Infinity, ease: "easeInOut" } }} />
        </svg>
      </div>
    </section>
  );
